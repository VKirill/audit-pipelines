...
> Раньше здесь была одна большая спецификация (40 КБ). С `chained-v2` пайплайн разбит на **13 поэтапных файлов в `phases/`**, каждый отвечает за свою фазу.
>
> Этот файл — индекс, чтобы быстро найти нужный.
...
- **autonomous-v1** (старая) — один большой `AUDIT.md`. Сохранён в git history.
- **chained-v2** (текущая) — 13 phase-файлов в `phases/`, MCP-first, hot-spots, DDD-словарь, Trade-off matrix, 4 типа output-артефактов.

`_meta.json` содержит поле `version` для backwards compat.
