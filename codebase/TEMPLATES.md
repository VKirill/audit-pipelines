# TEMPLATES — форматы артефактов (v3)
...
> **v3 ввёл два обязательных поля:** `confidence_rationale` (для каждого `high` finding) и `exploit_proof` (для каждого `critical` finding). Без них валидатор отклоняет finding.
...
- `category` — одна из: `architecture`, `dependencies`, `quality`, `error-handling`, `security`, `tests`, `ops`, `performance`, `dx`, `docs`, `data-flow` (новое в v3, для phase 02b), `money-safety` (новое в v3, для phase 06b).
...
- `confidence_rationale` (НОВОЕ в v3, обязательно при `confidence=high`) — одно-два предложения, описывающие **что именно** ты прочитал и **какой инструмент** это подтвердил. Формула: «Прочитал X (где), [запустил Y], не зависит от рантайма потому что Z».
- `exploit_proof` (НОВОЕ в v3, обязательно при `severity=critical`) — конкретный сценарий эксплуатации: HTTP-вызов / SQL-payload / последовательность шагов, которая ведёт к ущербу. Не «теоретически возможно», а «curl -X POST … → 200 → у пользователя списаны средства».
...
- `status` (НОВОЕ в v3) — `"open"` (по умолчанию) или `"merged"` (когда finding объединён в другой через дедупликацию в фазе 10). Для merged обязательно поле `merged_into: "F-NNNN"`. Не удаляй файл — пометь.
...
## 6. Проверка формата в конце каждой фазы (v3 — детерминированная)
...
## 7. Финальный артефакт — `audit/_meta.json` (НОВОЕ в v3)
...
## 8. Финальные артефакты (НОВОЕ в v3)
