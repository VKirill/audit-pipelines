...
if [[ "$REFRESH" == "1" ]]; then
  ...
  PROMPT_FILE="$SCRIPT_DIR/_staging/refresh.md"
  {
    ...
    echo
    echo "Existing manifest: \`$SCRIPT_DIR/manifest.yml\`"
    echo "Project root: \`$PROJECT_ROOT\`"
    echo
    echo "Output: updated manifest with refresh_state populated."
    echo
    echo "Validate when done:"
    echo '  python3 '"$SCRIPT_DIR"'/validators/validate_manifest.py $SCRIPT_DIR/manifest.yml --strict'
  } > "$PROMPT_FILE"
...
# Already exists?
if [[ -f "$SCRIPT_DIR/manifest.yml" ]]; then
  c_yellow "Existing manifest detected at $SCRIPT_DIR/manifest.yml"
  echo "Options:"
...
PROMPT_FILE="$SCRIPT_DIR/_staging/init.md"
{
  ...
  echo
  echo "Produce **\`$SCRIPT_DIR/manifest.yml\`** at project root, conforming to:"
...
echo "  2. Send: Прочитай $PROMPT_FILE и выполни discover-фазу. Создай $SCRIPT_DIR/manifest.yml."
echo "  3. Review $SCRIPT_DIR/manifest.yml"
